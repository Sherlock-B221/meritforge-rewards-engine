...
| D15 | **Public reads / auth writes** — feed + thread detail public + SSR; writes + engine/challenge/reward endpoints auth-gated | Real SEO on indexable content; documented enhancement over brief-literal. Resolves O7. `AD-9`,`02` |
...
| ~~O7~~ | ~~SEO scope: how public is the forum?~~ | — | **RESOLVED (D15)** — public reads / auth writes (feed + thread detail public + SSR). |
...
- **2026-08-15 (cont.)** — Designed BE + FE folder structures (D9–D14) → `08-backend-structure.md`,
  `09-frontend-structure.md`. Resolved O1, O7 (D15: public reads / auth writes), O2 (D16: rings +
  heatmap). Remaining opens (O3 deploy, O5 rich-text, O6 trending) are non-blocking. Next: formal
  spec + implementation plan → scaffold. No code scaffolded yet.
